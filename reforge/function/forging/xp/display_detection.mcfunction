scoreboard players operation .apply_cost const = .xp_cost const
...
execute at @s as @e[tag=target_type_reforge,sort=nearest,limit=1] if score .apply_id const matches 82 store result score .apply_cost const run data get entity @p XpLevel
execute at @s as @e[tag=target_type_reforge,sort=nearest,limit=1] if score .apply_id const matches 82 if score .apply_cost const matches 51.. run scoreboard players set .apply_cost const 50
