scoreboard players reset .detect_xp_cost const
execute store result score .detect_xp_cost const run data get entity @n[type=item,tag=iteminit] Item.components.minecraft:custom_data.forgecraft.xp_cost
...
execute if entity @s[advancements={reforge:forge_all_unique=false}] if score .reforge_length const matches 85.. run playsound minecraft:ui.toast.challenge_complete master @s ~ ~ ~ 1 0.95 0
execute if entity @s[advancements={reforge:forge_all_unique=false}] if score .reforge_length const matches 85.. run advancement grant @s only reforge:forge_all_unique
...
execute if entity @s[advancements={reforge:absolute_dedication=false}] if score .item_reforge reforgeId matches 6 if entity @n[type=item_display,limit=1,tag=targetforgeitem,nbt={item:{id:"minecraft:golden_hoe"}}] run advancement grant @s only reforge:absolute_dedication
