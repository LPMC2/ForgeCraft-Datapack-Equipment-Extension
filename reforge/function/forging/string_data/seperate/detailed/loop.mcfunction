...
$data modify storage forgecraft:string storage.seperated append value {"id":$(list_index)}
$data modify storage forgecraft:string storage.seperated[$(list_index)].char.letter set string storage forgecraft:string target.text $(index) $(index_offset)
...
execute if score .string_length const > .string_index const run function reforge:forging/string_data/seperate/detailed/loop with storage forgecraft:string storage