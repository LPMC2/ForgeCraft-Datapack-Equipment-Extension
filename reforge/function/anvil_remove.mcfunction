execute if entity @n[type=item,tag=forgecraft_anvil_item,distance=..1.5] run return run kill @e[tag=forge_tag,distance=..2]
scoreboard players set .anvil_count const 0
execute as @e[type=item,tag=forgecraft_anvil_item] run scoreboard players add .anvil_count const 1
execute if score .anvil_count const matches 20.. run return run kill @e[tag=forge_tag,distance=..2]
tag @s add forge_remove
execute as @e[tag=displayitem] at @s if score @s forgecraft.id = @e[tag=forge_remove,limit=1] forgecraft.id run function reforge:item_remove
