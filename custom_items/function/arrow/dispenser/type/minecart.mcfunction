summon minecart ~ ~ ~ {Tags:["throw_item","init"]}
...
execute as @n[type=minecart,tag=throw_item,tag=init] at @s run function custom_items:arrow/dispenser/set_projectile with storage minecraft:arrow store
kill @s
execute as @p[tag=arrow_owner] at @s run function custom_items:arrow/dispenser/remove_item
tag @n[type=minecart,tag=throw_item,tag=init] remove init