...
scoreboard players operation .id const = @s forgecraft.id 
execute at @s if score @s shielding_charges matches ..0 run execute as @e[type=!#entity_hit_matching:non_mob,distance=..5] at @s unless score .id const = @s forgecraft.id run damage @s 5 minecraft:player_explosion at ~ ~ ~
