...
execute if score .current_health const >= .max_health const unless entity @s[nbt={foodLevel:20}] run scoreboard players set @s healthful_cd 10
...
execute at @s run playsound minecraft:block.conduit.activate master @a ~ ~ ~ 1 2 0
